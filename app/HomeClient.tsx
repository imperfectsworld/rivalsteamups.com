                  ...
                  return (
                    <article className={`hero-panel ${enhanced ? "hero-enhanced" : ""}`} id={`hero-${hero.id}`} key={hero.id}>
                      <div className="hero-panel-header">
                        ...
                        <div className="hero-identity-actions"><a className="hero-details-link" href={path(`/heroes/${hero.id}`)}>+ {tx("DETAILS")}</a><button className={`favorite-toggle ${favoriteHeroIds.includes(hero.id) ? "is-active" : ""}`} type="button" aria-pressed={favoriteHeroIds.includes(hero.id)} onClick={() => toggleFavorite(hero.id)} disabled={!favoriteHeroIds.includes(hero.id) && favoriteHeroIds.length >= 5} aria-label={`${favoriteHeroIds.includes(hero.id) ? "Stop following" : "Follow"} ${localizedHeroName(hero)}`}>{favoriteHeroIds.includes(hero.id) ? "★ MAIN" : "☆ FOLLOW"}</button></div>
