...
export default async function Page({ params }: { params: Promise<{ heroId: string }> }) { const { heroId } = await params; const hero = heroes.find((item) => item.id === heroId); if (!hero) notFound(); return <HeroDetailClient hero={hero} locale="es" />; }
